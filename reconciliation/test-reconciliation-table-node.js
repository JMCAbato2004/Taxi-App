/**
 * Pruebas unitarias para ReconciliationTable - Versión Node.js
...
// Cargar el componente ReconciliationTable
const fs = require('fs');
const path = require('path');

// Leer y evaluar el archivo ReconciliationTable
const reconciliationTablePath = path.join(__dirname, 'reconciliation-table.js');
const reconciliationTableCode = fs.readFileSync(reconciliationTablePath, 'utf8');

// Evaluar el código en el contexto global
eval(reconciliationTableCode);
...
function runReconciliationTableTests() {
  ...
  // 7. PRUEBAS DE ESTRUCTURA
  console.log('🏗️ VALIDANDO ESTRUCTURA DE COMPONENTES');
  console.log('-'.repeat(40));

  test('Estructura de ReconciliationTable es correcta', () => {
    const result = ReconciliationTable({
      theme: mockTheme,
      reconciliation: mockReconciliation
    });
    
    assert(result.type === 'div', 'Componente principal debe ser div');
    assert(result.props.className === 'space-y-6', 'Debe tener clase de espaciado');
  });

  test('Todas las secciones tienen estructura correcta', () => {
    const sections = [
      DailyTotalsSection,
      SummarySection,
      SettlementSection,
    ...
    sections.forEach(Section => {
      assert(typeof Section === 'function', `${Section.name} debe ser una función`);
    });
  });
  ...
  if (passedTests === totalTests) {
    ...
    console.log('✅ Estructura de componentes validada');
  } else {
...
const result = runReconciliationTableTests();
