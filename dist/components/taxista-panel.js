...
export class TaxistaPanel {
    ...
    async initialize() {
        try {
            ...
            if (!user || user.rol !== UserRole.TAXISTA) {
                throw new AuthError(AuthErrorCode.INSUFFICIENT_PERMISSIONS, 'Solo los taxistas pueden acceder a este panel');
            }
    ...
    async loadPersonalProfile() {
        ...
        try {
            ...
            throw new AuthError(AuthErrorCode.NETWORK_ERROR, 'Error al cargar perfil personal', error);
        }
    }
    /**
     ...
     */
    async loadPersonalHistory(filters) {
        if (!this.currentUser)
            return;
        try {
            ...
            throw new AuthError(AuthErrorCode.NETWORK_ERROR, 'Error al cargar historial personal', error);
        }
    }
    /**
     * Calculate personal statistics
    ...
    async calculatePersonalStats() {
        ...
        try {
            ...
            const monthlyServices = this.personalServices.filter(s => new Date(s.date || '') >= currentMonth).length;
            ...
            throw new AuthError(AuthErrorCode.NETWORK_ERROR, 'Error al calcular estadísticas personales', error);
        }
    }
    /**
    ...
    applyHistoryFilters(filters) {
        ...
        // Date filtering
        if (filters.dateFrom) {
            filteredServices = filteredServices.filter(s => new Date(s.date || '') >= filters.dateFrom);
            filteredExpenses = filteredExpenses.filter(e => new Date(e.date || '') >= filters.dateFrom);
        }
        if (filters.dateTo) {
            filteredServices = filteredServices.filter(s => new Date(s.date || '') <= filters.dateTo);
            filteredExpenses = filteredExpenses.filter(e => new Date(e.date || '') <= filters.dateTo);
        }
        // Type filtering
 ...
 */
export function createTaxistaPanel(config) {
    return new TaxistaPanel(config);
}
/**
 * Hook-like function for React integration
 */
export function useTaxistaPanel(config) {
...
//# sourceMappingURL=taxista-panel.js.map