...
import { UserRole, AuthErrorCode, AuthError, ROLE_PERMISSIONS } from '../types';
...
export class AuthService {
    constructor(jwtUtils, cryptoUtils, sensitiveDataService, secureStorage) {
    ...
    async login(credentials) {
        try {
            ...
            if (!validation.isValid) {
                throw new AuthError(AuthErrorCode.VALIDATION_ERROR, validation.errors.map((e) => e.message).join(', '));
            }
            // In a real implementation, this would make an API call
            // For now, we'll simulate the authentication process
            const user = await this.authenticateUser(credentials);
            if (!user) {
                throw new AuthError(AuthErrorCode.INVALID_CREDENTIALS, 'Credenciales inválidas');
            }
            // Generate tokens
            ...
            throw new AuthError(AuthErrorCode.NETWORK_ERROR, 'Error durante el inicio de sesión', error);
        }
    }
    async logout() {
        try {
    ...
    async register(userData) {
        try {
            ...
            if (await this.emailExists(userData.email)) {
                throw new AuthError(AuthErrorCode.DUPLICATE_EMAIL, 'El email ya está registrado');
            }
            // Hash password
            ...
            throw new AuthError(AuthErrorCode.NETWORK_ERROR, 'Error durante el registro', error);
        }
    }
    getCurrentUser() {
    ...
    async refreshToken() {
        try {
            ...
            if (!refreshToken) {
                throw new AuthError(AuthErrorCode.SESSION_EXPIRED, 'Sesión expirada');
            }
            ...
            if (!currentUser) {
                throw new AuthError(AuthErrorCode.SESSION_EXPIRED, 'Usuario no encontrado');
            }
            ...
            throw new AuthError(AuthErrorCode.NETWORK_ERROR, 'Error al renovar token', error);
        }
    }
     ...
     */
    async storeCriticalData(key, data, maxAge) {
        try {
            ...
            throw new AuthError(AuthErrorCode.NETWORK_ERROR, `Error storing critical data: ${key}`, error);
        }
    }
    /**
    ...
    async syncOfflineData() {
        try {
            ...
            await this.secureStorage.storeOfflineAuthData({
                ...
                permissions: updatedOfflineData.permissions.map(p => p)
            });
        }
        catch (error) {
            console.error('Error syncing offline data:', error);
            throw new AuthError(AuthErrorCode.NETWORK_ERROR, 'Error syncing offline data', error);
        }
    }
    async changePassword(currentPassword, newPassword) {
        ...
        try {
            ...
            const isCurrentPasswordValid = await this.cryptoUtils.comparePassword(currentPassword, user.passwordHash);
            if (!isCurrentPasswordValid) {
                throw new AuthError(AuthErrorCode.INVALID_CREDENTIALS, 'Contraseña actual incorrecta');
            }
            ...
            if (passwordErrors.length > 0) {
                throw new AuthError(AuthErrorCode.VALIDATION_ERROR, passwordErrors.map((e) => e.message).join(', '));
            }
            ...
            throw new AuthError(AuthErrorCode.NETWORK_ERROR, 'Error al cambiar contraseña', error);
        }
    }
    /**
     ...
     */
    async initiatePasswordChange(currentPassword, newPassword) {
        if (!this.currentUser) {
            throw new AuthError(AuthErrorCode.SESSION_EXPIRED, 'Usuario no autenticado');
        }
        try {
            ...
            return await this.sensitiveDataService.initiateConfirmation(this.currentUser, SensitiveOperationType.PASSWORD_CHANGE, { currentPassword, newPassword });
        }
        catch (error) {
            if (error instanceof AuthError) {
                throw error;
            }
            throw new AuthError(AuthErrorCode.NETWORK_ERROR, 'Error al iniciar cambio de contraseña', error);
        }
    }
    /**
    ...
    async initiateEmailChange(newEmail) {
        ...
        try {
            ...
            if (!newEmail || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(newEmail)) {
                throw new AuthError(AuthErrorCode.VALIDATION_ERROR, 'Formato de email inválido');
            }
            // Check if email is already in use
            if (await this.emailExists(newEmail)) {
                throw new AuthError(AuthErrorCode.DUPLICATE_EMAIL, 'El email ya está en uso');
            }
            // Initiate confirmation process
            return await this.sensitiveDataService.initiateConfirmation(this.currentUser, SensitiveOperationType.EMAIL_CHANGE, { newEmail, currentEmail: this.currentUser.email });
        }
        catch (error) {
            if (error instanceof AuthError) {
                throw error;
            }
            throw new AuthError(AuthErrorCode.NETWORK_ERROR, 'Error al iniciar cambio de email', error);
        }
    }
    /**
     ...
     * Requirements: 6.4 - Additional confirmation for sensitive data
     */
    async initiateProfileUpdate(updateData) {
        if (!this.currentUser) {
            throw new AuthError(AuthErrorCode.SESSION_EXPIRED, 'Usuario no autenticado');
        }
        try {
            ...
            if (updateData.email && updateData.email !== this.currentUser.email) {
                ...
                throw new AuthError(AuthErrorCode.VALIDATION_ERROR, 'Use initiateEmailChange para cambiar el email');
            }
            ...
            if (!isSensitiveUpdate) {
                throw new AuthError(AuthErrorCode.VALIDATION_ERROR, 'No hay cambios sensibles que requieran confirmación');
            }
            // Initiate confirmation process
            return await this.sensitiveDataService.initiateConfirmation(this.currentUser, SensitiveOperationType.PROFILE_UPDATE, { updateData, currentData: {
            ...
            throw new AuthError(AuthErrorCode.NETWORK_ERROR, 'Error al iniciar actualización de perfil', error);
        }
    }
    /**
     ...
     * Requirements: 6.4 - Additional confirmation for sensitive data
     */
    async executeConfirmedPasswordChange(confirmationRequestId) {
        if (!this.currentUser) {
            throw new AuthError(AuthErrorCode.SESSION_EXPIRED, 'Usuario no autenticado');
        }
        try {
            // Execute the confirmed operation
            const result = await this.sensitiveDataService.executeConfirmedOperation(confirmationRequestId, this.currentUser);
            ...
            throw new AuthError(AuthErrorCode.NETWORK_ERROR, 'Error al ejecutar cambio de contraseña confirmado', error);
        }
    }
    /**
     ...
     * Requirements: 6.4 - Additional confirmation for sensitive data
     */
    async executeConfirmedEmailChange(confirmationRequestId) {
        if (!this.currentUser) {
            throw new AuthError(AuthErrorCode.SESSION_EXPIRED, 'Usuario no autenticado');
        }
        try {
            ...
            throw new AuthError(AuthErrorCode.NETWORK_ERROR, 'Error al ejecutar cambio de email confirmado', error);
        }
    }
    /**
     ...
     * Requirements: 6.4 - Additional confirmation for sensitive data
     */
    async executeConfirmedProfileUpdate(confirmationRequestId) {
        if (!this.currentUser) {
            throw new AuthError(AuthErrorCode.SESSION_EXPIRED, 'Usuario no autenticado');
        }
        try {
            ...
            throw new AuthError(AuthErrorCode.NETWORK_ERROR, 'Error al ejecutar actualización de perfil confirmada', error);
        }
    }
