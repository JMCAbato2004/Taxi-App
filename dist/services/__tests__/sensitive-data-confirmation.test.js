 ...
 */
import { SensitiveDataConfirmationService, SensitiveOperationType, ConfirmationMethod } from '../sensitive-data-confirmation';
...
describe('SensitiveDataConfirmationService', () => {
    ...
    describe('initiateConfirmation', () => {
        it('should create a confirmation request for password change', async () => {
            ...
            const request = await service.initiateConfirmation(testUser, SensitiveOperationType.PASSWORD_CHANGE, operationData);
        ...
        it('should create a confirmation request for email change with multiple methods', async () => {
            ...
            const request = await service.initiateConfirmation(testUser, SensitiveOperationType.EMAIL_CHANGE, operationData);
            expect(request.requiredMethods).toContain(ConfirmationMethod.PASSWORD_VERIFICATION);
        ...
        it('should create a confirmation request for association creation', async () => {
            ...
            const request = await service.initiateConfirmation(testUser, SensitiveOperationType.ASSOCIATION_CREATE, operationData);
        ...
        it('should reject confirmation for taxista trying to manage associations', async () => {
            ...
            await expect(service.initiateConfirmation(taxistaUser, SensitiveOperationType.ASSOCIATION_CREATE, {})).rejects.toThrow(AuthError);
        });
    });
    describe('processConfirmation', () => {
        let confirmationRequest;
        beforeEach(async () => {
            confirmationRequest = await service.initiateConfirmation(testUser, SensitiveOperationType.PASSWORD_CHANGE, { currentPassword: 'oldpass', newPassword: 'newpass123' });
        });
        it('should successfully process password verification', async () => {
            ...
            const result = await service.processConfirmation(confirmationRequest.id, attempt, testUser);
            ...
            expect(mockCryptoUtils.comparePassword).toHaveBeenCalledWith('correct_password', 'hashed_password_123');
        });
        ...
        it('should reject confirmation for non-existent request', async () => {
            ...
            await expect(service.processConfirmation('non_existent_id', attempt, testUser)).rejects.toThrow(AuthError);
        });
        it('should reject confirmation for wrong user', async () => {
            ...
            await expect(service.processConfirmation(confirmationRequest.id, attempt, otherUser)).rejects.toThrow(AuthError);
        });
        it('should handle expired confirmation request', async () => {
            ...
            const request = requests.find((r) => r.id === confirmationRequest.id);
            if (request) {
            ...
            await expect(service.processConfirmation(confirmationRequest.id, attempt, testUser)).rejects.toThrow(AuthError);
        });
    ...
    describe('executeConfirmedOperation', () => {
        ...
        it('should execute confirmed operation successfully', async () => {
            const result = await service.executeConfirmedOperation(confirmationRequest.id, testUser);
        ...
        it('should reject execution for incomplete confirmation', async () => {
            ...
            const incompleteRequest = await service.initiateConfirmation(testUser, SensitiveOperationType.EMAIL_CHANGE, { newEmail: 'new@example.com' });
            await expect(service.executeConfirmedOperation(incompleteRequest.id, testUser)).rejects.toThrow(AuthError);
        });
        it('should reject execution for wrong user', async () => {
            const otherUser = { ...testUser, id: 'other_user' };
            await expect(service.executeConfirmedOperation(confirmationRequest.id, otherUser)).rejects.toThrow(AuthError);
        });
    });
    describe('getConfirmationStatus', () => {
        let confirmationRequest;
        beforeEach(async () => {
            confirmationRequest = await service.initiateConfirmation(testUser, SensitiveOperationType.EMAIL_CHANGE, { newEmail: 'new@example.com' });
        });
    ...
    describe('cancelConfirmation', () => {
        ...
        it('should handle cancellation of non-existent request', async () => {
            ...
            await expect(service.cancelConfirmation('non_existent', testUser)).resolves.not.toThrow();
        });
        it('should reject cancellation by wrong user', async () => {
            const otherUser = { ...testUser, id: 'other_user' };
            await expect(service.cancelConfirmation(confirmationRequest.id, otherUser)).rejects.toThrow(AuthError);
        });
    });
    describe('Email verification simulation', () => {
        it('should generate and verify email codes', async () => {
            const request = await service.initiateConfirmation(testUser, SensitiveOperationType.EMAIL_CHANGE, { newEmail: 'new@example.com' });
    ...
    describe('Integration with different operation types', () => {
        ...
        it('should handle association removal confirmation with additional verification', async () => {
            ...
            const request = await service.initiateConfirmation(testUser, SensitiveOperationType.ASSOCIATION_REMOVE, operationData);
        ...
        it('should handle profile update confirmation', async () => {
            ...
            const request = await service.initiateConfirmation(testUser, SensitiveOperationType.PROFILE_UPDATE, operationData);
    ...
    describe('Error handling', () => {
        it('should handle storage errors gracefully', async () => {
            ...
            try {
                const request = await service.initiateConfirmation(testUser, SensitiveOperationType.PASSWORD_CHANGE, { currentPassword: 'old', newPassword: 'new' });
...
//# sourceMappingURL=sensitive-data-confirmation.test.js.map