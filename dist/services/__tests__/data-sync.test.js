 ...
 */
import { DataSyncService, SyncOperationType, SyncOperationStatus, ConflictResolutionStrategy } from '../data-sync';
...
describe('DataSyncService', () => {
    ...
    let mockRoleService;
    let mockServiceExpenseIntegration;
    ...
    beforeEach(() => {
        ...
        dataSyncService = new DataSyncService(mockSecureStorage, mockRoleService, mockServiceExpenseIntegration, mockGetCurrentUser);
    });
    afterEach(() => {
    ...
    describe('Queue Management', () => {
        it('should queue operation successfully', async () => {
            ...
            const operationId = await dataSyncService.queueOperation(SyncOperationType.CREATE_SERVICE, serviceData);
            expect(operationId).toBeDefined();
            expect(operationId).toMatch(/^sync_\d+_[a-z0-9]+$/);
            expect(mockSecureStorage.storeCriticalData).toHaveBeenCalledWith('sync_queue', expect.any(Array));
        });
        it('should throw error when user not authenticated', async () => {
            mockGetCurrentUser.mockReturnValue(null);
            await expect(dataSyncService.queueOperation(SyncOperationType.CREATE_SERVICE, {})).rejects.toThrow(AuthError);
        });
        it('should throw error when queue is full', async () => {
            ...
            await expect(dataSyncService.queueOperation(SyncOperationType.CREATE_SERVICE, {})).rejects.toThrow('Cola de sincronización llena');
        });
        ...
        it('should give higher priority to patron operations when configured', async () => {
            ...
            dataSyncService.destroy();
            dataSyncService = new DataSyncService(mockSecureStorage, mockRoleService, mockServiceExpenseIntegration, mockGetCurrentUser, { prioritizeUserRole: true });
    ...
    describe('Operation Cancellation', () => {
        let operationId;
        beforeEach(async () => {
            operationId = await dataSyncService.queueOperation(SyncOperationType.CREATE_SERVICE, { id: 'service-1' });
        });
        ...
        it('should throw error when operation not found', async () => {
            await expect(dataSyncService.cancelOperation('non-existent-id')).rejects.toThrow('Operación no encontrada');
        });
        it('should throw error when user not authenticated', async () => {
            mockGetCurrentUser.mockReturnValue(null);
            await expect(dataSyncService.cancelOperation(operationId)).rejects.toThrow(AuthError);
        });
        it('should throw error when user lacks permission', async () => {
            ...
            await expect(dataSyncService.cancelOperation(patronOperationId)).rejects.toThrow('Sin permisos para cancelar esta operación');
        });
    });
    ...
    describe('Conflict Resolution', () => {
        it('should store conflicts for manual resolution', async () => {
            ...
            await dataSyncService.queueOperation(SyncOperationType.UPDATE_SERVICE, { name: 'Local Name', amount: 100 }, { conflictResolution: ConflictResolutionStrategy.MANUAL });
            expect(mockSecureStorage.storeCriticalData).toHaveBeenCalled();
        });
        it('should resolve conflict with client wins strategy', async () => {
            const operationId = await dataSyncService.queueOperation(SyncOperationType.UPDATE_SERVICE, { name: 'Local Name' });
            const conflict = {
                operationId,
                type: SyncOperationType.UPDATE_SERVICE,
            ...
            await dataSyncService.resolveConflict(operationId, ConflictResolutionStrategy.CLIENT_WINS);
            const pendingOps = dataSyncService.getPendingOperations();
        ...
        it('should resolve conflict with server wins strategy', async () => {
            ...
            mockSecureStorage.getCriticalData.mockResolvedValue([conflict]);
            await dataSyncService.resolveConflict(operationId, ConflictResolutionStrategy.SERVER_WINS);
            const pendingOps = dataSyncService.getPendingOperations();
            const resolvedOp = pendingOps.find(op => op.id === operationId);
            expect(resolvedOp?.data.name).toBe('Server Name');
        });
        it('should resolve conflict with merge strategy', async () => {
            const operationId = await dataSyncService.queueOperation(SyncOperationType.UPDATE_SERVICE, { name: 'Local Name', userField: 'local' });
            const conflict = {
                operationId,
                type: SyncOperationType.UPDATE_SERVICE,
            ...
            mockSecureStorage.getCriticalData.mockResolvedValue([conflict]);
            await dataSyncService.resolveConflict(operationId, ConflictResolutionStrategy.MERGE);
            const pendingOps = dataSyncService.getPendingOperations();
            const resolvedOp = pendingOps.find(op => op.id === operationId);
            expect(resolvedOp?.status).toBe(SyncOperationStatus.PENDING);
        });
        it('should throw error when conflict not found', async () => {
            mockSecureStorage.getCriticalData.mockResolvedValue([]);
            await expect(dataSyncService.resolveConflict('non-existent', ConflictResolutionStrategy.CLIENT_WINS)).rejects.toThrow('Conflicto no encontrado');
        });
    });
    ...
    describe('Configuration Management', () => {
        it('should update configuration successfully', async () => {
            ...
            expect(mockSecureStorage.storeCriticalData).toHaveBeenCalledWith('sync_config', expect.objectContaining(newConfig));
        });
    ...
    describe('Error Handling', () => {
        ...
        it('should handle invalid operation types', async () => {
            ...
            if (pendingOps.length > 0) {
                pendingOps[0].type = 'INVALID_TYPE';
            }
...
//# sourceMappingURL=data-sync.test.js.map