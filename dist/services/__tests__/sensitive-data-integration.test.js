...
describe('Sensitive Data Confirmation Integration', () => {
    ...
    describe('Password Change with Confirmation', () => {
        it('should require confirmation for password change', async () => {
            const confirmationRequest = await authService.initiatePasswordChange('Password123!', 'NewPassword456!');
        ...
        it('should complete password change after confirmation', async () => {
            ...
            const confirmationResult = await sensitiveDataService.processConfirmation(confirmationRequest.id, {
        ...
        it('should reject password change with incorrect current password', async () => {
            ...
            await expect(authService.initiatePasswordChange('WrongPassword!', 'NewPassword456!')).rejects.toThrow(AuthError);
        });
        it('should reject password change with invalid new password format', async () => {
            await expect(authService.initiatePasswordChange('Password123!', '123') // Too short
    ...
    describe('Email Change with Confirmation', () => {
        ...
        it('should complete email change after full confirmation', async () => {
            ...
            await sensitiveDataService.processConfirmation(confirmationRequest.id, {
                method: ConfirmationMethod.PASSWORD_VERIFICATION,
                data: { password: 'Password123!' }
            }, patronUser);
            ...
            const emailResult = await sensitiveDataService.processConfirmation(confirmationRequest.id, {
        ...
        it('should reject email change to existing email', async () => {
            await expect(authService.initiateEmailChange(taxistaUser.email)).rejects.toThrow(AuthError);
        });
        it('should reject email change with invalid format', async () => {
            await expect(authService.initiateEmailChange('invalid-email')).rejects.toThrow(AuthError);
        });
    });
    describe('Profile Update with Confirmation', () => {
        ...
        it('should reject profile update that includes email change', async () => {
            ...
            await expect(authService.initiateProfileUpdate(updateData)).rejects.toThrow(AuthError);
        });
    });
    describe('Association Management with Confirmation', () => {
        it('should require confirmation for association creation', async () => {
            const confirmationRequest = await roleService.initiateAssociationCreation(patronUser.id, taxistaUser.id);
            expect(confirmationRequest).toBeDefined();
        ...
        it('should reject association operations for taxistas', async () => {
            ...
            await expect(roleService.initiateAssociationCreation(patronUser.id, taxistaUser.id)).rejects.toThrow(AuthError);
        });
    ...
    describe('Confirmation Status and Management', () => {
        it('should track confirmation status correctly', async () => {
            ...
            let status = await sensitiveDataService.getConfirmationStatus(confirmationRequest.id, patronUser);
            ...
            status = await sensitiveDataService.getConfirmationStatus(confirmationRequest.id, patronUser);
        ...
        it('should allow cancellation of confirmation requests', async () => {
            ...
            // Verify it's gone
            const status = await sensitiveDataService.getConfirmationStatus(confirmationRequest.id, patronUser);
            expect(status.exists).toBe(false);
        });
        it('should handle multiple concurrent confirmation requests', async () => {
            const passwordRequest = await authService.initiatePasswordChange('Password123!', 'NewPassword456!');
    ...
    describe('Error Handling and Edge Cases', () => {
        it('should handle confirmation attempts after expiration', async () => {
            ...
            await expect(sensitiveDataService.processConfirmation(confirmationRequest.id, {
                method: ConfirmationMethod.PASSWORD_VERIFICATION,
                data: { password: 'password123' }
            }, patronUser)).rejects.toThrow(AuthError);
        });
        it('should handle execution attempts on expired confirmations', async () => {
            ...
            await expect(authService.executeConfirmedPasswordChange(confirmationRequest.id)).rejects.toThrow(AuthError);
        });
        ...
        it('should handle unauthenticated users', async () => {
            ...
            await expect(authService.initiatePasswordChange('old', 'new')).rejects.toThrow(AuthError);
            await expect(roleService.initiateAssociationCreation('patron_id', 'taxista_id')).rejects.toThrow(AuthError);
        });
    });
});
//# sourceMappingURL=sensitive-data-integration.test.js.map