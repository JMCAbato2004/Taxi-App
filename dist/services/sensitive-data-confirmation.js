 ...
 */
export var SensitiveOperationType;
(function (SensitiveOperationType) {
    SensitiveOperationType["PASSWORD_CHANGE"] = "password_change";
    SensitiveOperationType["EMAIL_CHANGE"] = "email_change";
    SensitiveOperationType["PROFILE_UPDATE"] = "profile_update";
    SensitiveOperationType["ASSOCIATION_CREATE"] = "association_create";
    SensitiveOperationType["ASSOCIATION_REMOVE"] = "association_remove";
    SensitiveOperationType["ACCOUNT_DEACTIVATION"] = "account_deactivation";
    SensitiveOperationType["ROLE_CHANGE"] = "role_change";
    SensitiveOperationType["SENSITIVE_DATA_ACCESS"] = "sensitive_data_access";
...
(function (ConfirmationMethod) {
    ConfirmationMethod["PASSWORD_VERIFICATION"] = "password_verification";
    ConfirmationMethod["EMAIL_VERIFICATION"] = "email_verification";
    ConfirmationMethod["TWO_FACTOR_AUTH"] = "two_factor_auth";
    ConfirmationMethod["SECURITY_QUESTION"] = "security_question";
...
export class SensitiveDataConfirmationService {
    constructor(cryptoUtils, authorizationMiddleware) {
        this.CONFIRMATION_REQUESTS_KEY = 'taxi_confirmation_requests';
        this.CONFIRMATION_LOG_KEY = 'taxi_confirmation_log';
     ...
     */
    async initiateConfirmation(user, operationType, operationData, customConfig) {
        try {
            ...
            throw new AuthError(AuthErrorCode.NETWORK_ERROR, 'Error al iniciar confirmación de datos sensibles', error);
        }
    }
    /**
     * Process a confirmation attempt
     */
    async processConfirmation(requestId, attempt, user) {
        try {
            // Get the confirmation request
            const request = await this.getConfirmationRequest(requestId);
            if (!request) {
                throw new AuthError(AuthErrorCode.INVALID_TOKEN, 'Solicitud de confirmación no encontrada');
            }
            // Validate request ownership
            if (request.userId !== user.id) {
                throw new AuthError(AuthErrorCode.INSUFFICIENT_PERMISSIONS, 'No autorizado para esta confirmación');
            }
            // Check if request is expired
            if (new Date() > request.expiresAt) {
                await this.expireConfirmationRequest(requestId);
                throw new AuthError(AuthErrorCode.SESSION_EXPIRED, 'Solicitud de confirmación expirada');
            }
            // Check if request is already completed
            if (request.completed) {
                throw new AuthError(AuthErrorCode.VALIDATION_ERROR, 'Confirmación ya completada');
            }
            // Check if method is already completed
            if (request.completedMethods.includes(attempt.method)) {
                throw new AuthError(AuthErrorCode.VALIDATION_ERROR, 'Método de confirmación ya completado');
            }
            // Check if method is required
            if (!request.requiredMethods.includes(attempt.method)) {
                throw new AuthError(AuthErrorCode.VALIDATION_ERROR, 'Método de confirmación no requerido');
            }
            // Check attempt limits
            if (request.attempts >= request.maxAttempts) {
                await this.expireConfirmationRequest(requestId);
                throw new AuthError(AuthErrorCode.VALIDATION_ERROR, 'Máximo número de intentos excedido');
            }
            // Process the specific confirmation method
            const methodResult = await this.processConfirmationMethod(attempt, user, request);
            // Update request
            request.attempts += 1;
            if (methodResult.success) {
                request.completedMethods.push(attempt.method);
                // Check if all methods are completed
                const allMethodsCompleted = request.requiredMethods.every(method => request.completedMethods.includes(method));
            ...
            const remainingMethods = request.requiredMethods.filter(method => !request.completedMethods.includes(method));
            return {
                success: methodResult.success,
                requestId,
            ...
            throw new AuthError(AuthErrorCode.NETWORK_ERROR, 'Error al procesar confirmación', error);
        }
    }
    /**
     * Execute the confirmed sensitive operation
     */
    async executeConfirmedOperation(requestId, user) {
        try {
            const request = await this.getConfirmationRequest(requestId);
            if (!request) {
                throw new AuthError(AuthErrorCode.INVALID_TOKEN, 'Solicitud de confirmación no encontrada');
            ...
            // Validate request ownership
            if (request.userId !== user.id) {
                throw new AuthError(AuthErrorCode.INSUFFICIENT_PERMISSIONS, 'No autorizado para esta operación');
            }
            // Check if confirmation is completed
            if (!request.completed) {
                throw new AuthError(AuthErrorCode.VALIDATION_ERROR, 'Confirmación no completada');
            }
            // Check if request is expired
            if (new Date() > request.expiresAt) {
                await this.expireConfirmationRequest(requestId);
            ...
            throw new AuthError(AuthErrorCode.NETWORK_ERROR, 'Error al ejecutar operación confirmada', error);
        }
    }
    /**
     * Get confirmation request status
     */
    async getConfirmationStatus(requestId, user) {
        try {
            const request = await this.getConfirmationRequest(requestId);
            if (!request) {
                return {
            ...
            // Validate ownership
            if (request.userId !== user.id) {
                throw new AuthError(AuthErrorCode.INSUFFICIENT_PERMISSIONS, 'No autorizado para ver esta confirmación');
            }
            ...
            throw new AuthError(AuthErrorCode.NETWORK_ERROR, 'Error al obtener estado de confirmación', error);
        }
    }
    /**
     * Cancel a confirmation request
     */
    async cancelConfirmation(requestId, user) {
        try {
            const request = await this.getConfirmationRequest(requestId);
            if (!request) {
            ...
            // Validate ownership
            if (request.userId !== user.id) {
                throw new AuthError(AuthErrorCode.INSUFFICIENT_PERMISSIONS, 'No autorizado para cancelar esta confirmación');
            }
            // Remove the request
            await this.removeConfirmationRequest(requestId);
            // Log cancellation
            ...
            throw new AuthError(AuthErrorCode.NETWORK_ERROR, 'Error al cancelar confirmación', error);
        }
    }
    // Private helper methods
    async validateOperationPermissions(user, operationType, operationData) {
        ...
        switch (operationType) {
            ...
            case SensitiveOperationType.ASSOCIATION_REMOVE:
                if (user.rol !== UserRole.PATRON) {
                    throw new AuthError(AuthErrorCode.INSUFFICIENT_PERMISSIONS, 'Solo los patrones pueden gestionar asociaciones');
                }
                break;
            case SensitiveOperationType.ROLE_CHANGE:
                // Role changes might require admin permissions in a real system
                throw new AuthError(AuthErrorCode.INSUFFICIENT_PERMISSIONS, 'Cambio de rol no permitido');
            default:
                // Most operations are allowed for the user's own data
                break;
        }
    }
    async processConfirmationMethod(attempt, user, request) {
        try {
            switch (attempt.method) {
                ...
                default:
                    return {
                        success: false,
                        error: new AuthError(AuthErrorCode.VALIDATION_ERROR, 'Método de confirmación no soportado')
                    };
            }
        }
        catch (error) {
            return {
                success: false,
                error: error instanceof AuthError ? error : new AuthError(AuthErrorCode.NETWORK_ERROR, 'Error al verificar método de confirmación', error)
            };
        }
    }
    async verifyPassword(password, user) {
        try {
            ...
            return {
                success: false,
                error: new AuthError(AuthErrorCode.NETWORK_ERROR, 'Error al verificar contraseña', error)
            };
        }
    }
    async verifyEmailCode(code, user, request) {
    ...
    generateEmailVerificationCode(email, requestId) {
    ...
    async storeConfirmationRequest(request) {
        try {
    ...
    async getConfirmationRequest(requestId) {
        try {
            const requests = this.getStoredConfirmationRequests();
    ...
    async removeConfirmationRequest(requestId) {
        try {
            const requests = this.getStoredConfirmationRequests();
    ...
    async expireConfirmationRequest(requestId) {
        await this.removeConfirmationRequest(requestId);
    }
    getStoredConfirmationRequests() {
        try {
    ...
    async logConfirmationEvent(user, eventType, data) {
        try {
            const logEntry = {
                id: this.generateRequestId(),
                userId: user.id,
...
//# sourceMappingURL=sensitive-data-confirmation.js.map