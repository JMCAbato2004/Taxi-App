...
export class ValidationUtils {
    ...
    validateLoginCredentials(credentials) {
        ...
        if (errors.length > 0) {
            throw new AuthError(AuthErrorCode.VALIDATION_ERROR, 'Datos de inicio de sesión inválidos', { errors });
        }
    }
    validateRegisterData(userData) {
        ...
        if (errors.length > 0) {
            throw new AuthError(AuthErrorCode.VALIDATION_ERROR, 'Datos de registro inválidos', { errors });
        }
    }
    validatePassword(password) {
        const validation = this.validatePasswordStrength(password);
        if (!validation.isValid) {
            throw new AuthError(AuthErrorCode.VALIDATION_ERROR, 'La contraseña no cumple con los requisitos de seguridad', { errors: validation.errors });
        }
    }
    validateEmail(email) {
        if (!email) {
            throw new AuthError(AuthErrorCode.VALIDATION_ERROR, 'El email es obligatorio');
        }
        if (!this.isValidEmail(email)) {
            throw new AuthError(AuthErrorCode.VALIDATION_ERROR, 'El formato del email no es válido');
        }
    }
    validateName(name) {
        if (!name) {
            throw new AuthError(AuthErrorCode.VALIDATION_ERROR, 'El nombre es obligatorio');
        }
        if (!this.isValidName(name)) {
            throw new AuthError(AuthErrorCode.VALIDATION_ERROR, 'El nombre debe tener entre 2 y 50 caracteres y solo contener letras y espacios');
        }
    }
    validatePhone(phone) {
        if (phone && !this.isValidPhone(phone)) {
            throw new AuthError(AuthErrorCode.VALIDATION_ERROR, 'El formato del teléfono no es válido');
        }
    }
    validateRole(role) {
        if (!role) {
            throw new AuthError(AuthErrorCode.VALIDATION_ERROR, 'El rol es obligatorio');
        }
        if (!Object.values(UserRole).includes(role)) {
            throw new AuthError(AuthErrorCode.VALIDATION_ERROR, 'El rol especificado no es válido');
        }
    }
    sanitizeInput(input) {
        if (!input)
