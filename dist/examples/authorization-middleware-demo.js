 ...
 */
import { AuthorizationMiddleware, createAuthorizationContext } from '../middleware/authorization-middleware';
...
export class AuthorizationDemo {
     ...
     */
    async demoBasicPermissionCheck() {
        console.log('\n=== Demo 1: Basic Permission Checking ===');
        const patronUser = {
            id: 'patron_demo',
        ...
        const patronContext = createAuthorizationContext(patronUser, 'associations', 'create', Permission.MANAGE_ASSOCIATIONS);
        ...
        const taxistaContext = createAuthorizationContext(taxistaUser, 'associations', 'create', Permission.MANAGE_ASSOCIATIONS);
     ...
     */
    async demoDataAccessValidation() {
        ...
        for (const data of testData) {
            const canAccess = await this.authMiddleware.validateDataAccess(patronUser, data, 'read');
            console.log(`- Can access "${data.content}": ${canAccess}`);
        }
        console.log('Testing taxista data access:');
        for (const data of testData) {
            const canAccess = await this.authMiddleware.validateDataAccess(taxistaUser, data, 'read');
            console.log(`- Can access "${data.content}": ${canAccess}`);
        }
    }
    /**
    ...
    async demoDataEncryption() {
        ...
        const encryptedData = await this.authMiddleware.encryptSensitiveData(sensitiveData, {
            encryptFields: ['email', 'telefono'],
            skipEncryption: false
        });
        console.log('Encrypted data:', encryptedData);
        // Decrypt sensitive fields
        const decryptedData = await this.authMiddleware.decryptSensitiveData(encryptedData, {
            encryptFields: ['email', 'telefono'],
            skipEncryption: false
        });
    ...
    async demoProtectedOperations() {
        ...
        try {
            ...
            const result = await this.integration.requirePermission(patronUser, {
        ...
        try {
            ...
            await this.integration.requirePermission(taxistaUser, {
                permission: Permission.MANAGE_ASSOCIATIONS,
                resource: 'associations',
                action: 'create'
            }, createAssociationOperation);
    ...
    async demoDataFiltering() {
        ...
        try {
            const filteredData = await this.integration.validateAndFilterData(patronUser, allData, Permission.VIEW_ALL_DRIVERS, 'operational-data');
            console.log('Filtered data for patron:', filteredData);
        }
        catch (error) {
            console.log('Data filtering failed:', error.message);
        }
    }
    /**
     * Demo 6: Access logging and security monitoring
     */
    async demoAccessLogging() {
        console.log('\n=== Demo 6: Access Logging and Security Monitoring ===');
        ...
        for (let i = 0; i < 5; i++) {
            ...
            const context = createAuthorizationContext(user, `resource_${i}`, 'read', i % 2 === 0 ? Permission.VIEW_ALL_DRIVERS : Permission.MANAGE_ASSOCIATIONS);
...
//# sourceMappingURL=authorization-middleware-demo.js.map