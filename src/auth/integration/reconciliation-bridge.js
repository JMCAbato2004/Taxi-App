...
class ReconciliationBridge {
  ...
  async initialize(authService, roleService) {
    try {
      ...
      this.reconciliationIntegration = new window.ReconciliationIntegrationService(
        this.roleService,
  ...
  filterServices(services) {
    if (!this.isReady() || !this.reconciliationIntegration) return [];
    try {
  ...
  getAggregatedSummary(data, aggregationField = 'amount') {
    if (!this.isReady() || !this.reconciliationIntegration) return null;
    try {
  ...
  getStatistics(storageManager) {
    ...
    try {
      ...
      if (user.rol === 'patron') {
        ...
        const aggregatedServices = this.getAggregatedSummary(
          storageManager.getServices(), 
          'totalAmount'
        );
        const aggregatedExpenses = this.getAggregatedSummary(
          storageManager.getExpenses(), 
          'amount'
        );
  ...
  function EnhancedReconciliationModule(props) {
        ...
        try {
          ...
          if (window.authServiceInstance && window.roleServiceInstance) {
            const initialized = await bridge.initialize(
              window.authServiceInstance, 
              window.roleServiceInstance
            );
    ...
    const enhancedProps = {
      ...
      onGenerateReconciliation: (calculationEngine, selectedPeriod, cashBreakdown, distributionSettings) => {
        if (isRoleAware) {
          return bridge.generateRoleAwareReconciliation(
            calculationEngine,
            props.services || [],
            props.expenses || [],
            selectedPeriod,
          ...
          return calculationEngine.generateReconciliation(
            props.services || [],
            props.expenses || [],
            selectedPeriod,
            cashBreakdown,
    ...
    const roleNotificationElements = roleNotifications.length > 0 ? 
      e('div', { className: "mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg" },
        e('h4', { className: "font-semibold text-blue-800 mb-2" }, '💡 Información de Rol'),
        roleNotifications.map((notification, index) =>
          e('p', { key: index, className: "text-sm text-blue-700 mb-1" }, notification)
        )
      ) : null;
    ...
    const aggregatedDataElement = aggregatedData && bridge.getUserRole() === 'patron' ?
      e('div', { className: "mb-6 p-4 bg-green-50 border border-green-200 rounded-lg" },
        e('h4', { className: "font-semibold text-green-800 mb-3" }, '📊 Resumen Agregado'),
        e('div', { className: "grid grid-cols-2 md:grid-cols-4 gap-4 text-sm" },
          e('div', { className: "text-center" },
            e('div', { className: "font-bold text-lg text-green-700" }, aggregatedData.associatedTaxistasCount),
            e('div', { className: "text-green-600" }, 'Taxistas')
          ),
          e('div', { className: "text-center" },
            e('div', { className: "font-bold text-lg text-green-700" }, aggregatedData.totalServices),
            e('div', { className: "text-green-600" }, 'Servicios')
          ),
          e('div', { className: "text-center" },
            e('div', { className: "font-bold text-lg text-green-700" }, `${aggregatedData.totalServiceAmount.toFixed(2)}€`),
            e('div', { className: "text-green-600" }, 'Ingresos')
          ),
          e('div', { className: "text-center" },
            e('div', { className: "font-bold text-lg text-green-700" }, `${aggregatedData.totalExpenseAmount.toFixed(2)}€`),
            e('div', { className: "text-green-600" }, 'Gastos')
          )
        )
      ) : null;
    ...
    return e('div', null,
      roleNotificationElements,
      aggregatedDataElement,
      e(originalReconciliationModule, enhancedProps)
    );
  }
