...
import { ServiceExpenseIntegrationService, AuthenticatedServiceData, AuthenticatedExpenseData } from '../service-expense-integration';
import { ReconciliationIntegrationService, ServiceData, ExpenseData } from '../reconciliation-integration';
import { RoleService } from '../role-service';
import { AuthService } from '../auth-service';
import { User, UserRole, Permission, AuthError, AuthErrorCode } from '../../types';
...
class MockRoleService {
  ...
  filterDataByRole<T extends { userId?: string; createdBy?: string; taxistaId?: string }>(
    data: T[], 
    options?: any
  ): T[] {
  ...
  validateDataAccess(data: any, operation: string): boolean {
...
describe('ServiceExpenseIntegrationService', () => {
  let service: ServiceExpenseIntegrationService;
  let mockRoleService: MockRoleService;
  let mockReconciliationService: ReconciliationIntegrationService;
  ...
  beforeEach(() => {
    ...
    mockReconciliationService = new ReconciliationIntegrationService(
      mockRoleService as any,
      mockGetCurrentUser
    );
    
    service = new ServiceExpenseIntegrationService(
      mockRoleService as any,
  ...
  describe('createService', () => {
    ...
    it('should throw error when user lacks permissions', async () => {
      ...
      (mockRoleService as any).permissions = [Permission.VIEW_OWN_DATA];
  ...
  describe('updateService', () => {
    const existingServices: ServiceData[] = [
      {
        id: 'service_1',
