...
export class RoleService implements IRoleService {
  private readonly ASSOCIATIONS_KEY = 'taxi_associations';
  ...
  async getAssociatedUsers(): Promise<User[]> {
    ...
    try {
      ...
      throw new AuthError(
        ...
        'Error al obtener usuarios asociados',
        error
      );
    }
  }

  async createAssociation(patronId: string, taxistaId: string): Promise<Association> {
    const currentUser = this.getCurrentUser();
    if (!currentUser) {
      throw new AuthError(
        AuthErrorCode.SESSION_EXPIRED,
    ...
    if (!this.hasPermission(Permission.MANAGE_ASSOCIATIONS)) {
      throw new AuthError(
        ...
        'Sin permisos para gestionar asociaciones'
      );
    }
    ...
    try {
      ...
      const existingAssociation = existingAssociations.find(
        a => a.patronId === patronId && a.taxistaId === taxistaId && a.activa
      );
      ...
      const taxistaAssociations = existingAssociations.filter(
        a => a.taxistaId === taxistaId && a.activa
      );

      if (taxistaAssociations.length > 0) {
        throw new AuthError(
          AuthErrorCode.INVALID_ASSOCIATION,
          'El taxista ya está asociado a otro patrón'
        );
      }
   ...
   */
  async initiateAssociationCreation(
    patronId: string, 
    taxistaId: string
    ...
    try {
      ...
      throw new AuthError(
        AuthErrorCode.NETWORK_ERROR,
        'Error al iniciar creación de asociación',
        error
      );
    }
  }
  ...
  async removeAssociation(associationId: string): Promise<void> {
    ...
    try {
      ...
      if (association && currentUser.rol === UserRole.PATRON && association.patronId !== currentUser.id) {
        throw new AuthError(
          AuthErrorCode.INSUFFICIENT_PERMISSIONS,
          'No tienes permisos para eliminar esta asociación'
        );
      }
   ...
   * Requirements: 6.4 - Additional confirmation for sensitive data
   */
  async initiateAssociationRemoval(associationId: string): Promise<ConfirmationRequest> {
    const currentUser = this.getCurrentUser();
    if (!currentUser) {
      throw new AuthError(
        AuthErrorCode.SESSION_EXPIRED,
    ...
    try {
      ...
      if (!patron || !taxista) {
        throw new AuthError(
          AuthErrorCode.USER_NOT_FOUND,
          'Usuarios de la asociación no encontrados'
        );
      }
      ...
      throw new AuthError(
        AuthErrorCode.NETWORK_ERROR,
        'Error al iniciar eliminación de asociación',
        error
      );
    }
  }
   ...
   * Requirements: 6.4 - Additional confirmation for sensitive data
   */
  async executeConfirmedAssociationCreation(confirmationRequestId: string): Promise<Association> {
    const currentUser = this.getCurrentUser();
    if (!currentUser) {
      throw new AuthError(
        AuthErrorCode.SESSION_EXPIRED,
    ...
    try {
      ...
      throw new AuthError(
        ...
        'Tipo de operación inválido para creación de asociación'
      );
      ...
      throw new AuthError(
        AuthErrorCode.NETWORK_ERROR,
        'Error al ejecutar creación de asociación confirmada',
        error
      );
    }
  }
   ...
   * Requirements: 6.4 - Additional confirmation for sensitive data
   */
  async executeConfirmedAssociationRemoval(confirmationRequestId: string): Promise<void> {
    const currentUser = this.getCurrentUser();
    if (!currentUser) {
      throw new AuthError(
        AuthErrorCode.SESSION_EXPIRED,
    ...
    try {
      ...
      if (result.operationType === SensitiveOperationType.ASSOCIATION_REMOVE) {
        ...
        throw new AuthError(
          AuthErrorCode.VALIDATION_ERROR,
          'Tipo de operación inválido para eliminación de asociación'
        );
      }
      ...
      throw new AuthError(
        AuthErrorCode.NETWORK_ERROR,
        'Error al ejecutar eliminación de asociación confirmada',
        error
      );
    }
  }
   ...
   */
  private filterDataForPatron<T>(data: T[], patron: User, userContext?: any): T[] {
    try {
      ...
      if (associatedTaxistaIds.length === 0) return [];
      ...
      return data.filter(item => {
        ...
        if (itemAny.numeroTaxista) {
          const users = this.getStoredUsers();
          const taxistaWithNumber = users.find(u => 
            u.numeroTaxista === itemAny.numeroTaxista && 
            associatedTaxistaIds.includes(u.id)
          );
          if (taxistaWithNumber) return true;
        }
   ...
   */
  private filterDataForTaxista<T>(data: T[], taxista: User, userContext?: any): T[] {
    try {
      return data.filter(item => {
        const itemAny = item as any;
  ...
  async searchAvailableTaxistas(searchTerm?: string): Promise<AvailableTaxista[]> {
    const currentUser = this.getCurrentUser();
    if (!currentUser || currentUser.rol !== UserRole.PATRON) {
      throw new AuthError(
        AuthErrorCode.INSUFFICIENT_PERMISSIONS,
        'Solo los patrones pueden buscar taxistas'
      );
    }
    ...
    try {
      ...
      let availableTaxistas = users.filter(u => 
        u.rol === UserRole.TAXISTA && 
        u.activo && 
        !associatedTaxistaIds.includes(u.id)
      );
      ...
      if (searchTerm) {
        ...
        availableTaxistas = availableTaxistas.filter(u =>
      ...
      throw new AuthError(
        AuthErrorCode.NETWORK_ERROR,
        'Error al buscar taxistas disponibles',
        error
      );
    }
  }

  async getAssociationsForPatron(patronId: string): Promise<Association[]> {
    const associations = this.getStoredAssociations();
    return associations.filter(a => a.patronId === patronId);
  }

  async getAssociationsForTaxista(taxistaId: string): Promise<Association[]> {
    const associations = this.getStoredAssociations();
  ...
  getAccessibleUsers(): User[] {
    ...
    if (currentUser.rol === UserRole.PATRON) {
      ...
      return allUsers.filter(u => 
        u.id === currentUser.id || associatedTaxistaIds.includes(u.id)
      );
    }
   ...
   */
  validateDataAccess(targetData: any, operation: 'read' | 'write' | 'delete' = 'read'): boolean {
    const currentUser = this.getCurrentUser();
    if (!currentUser) return false;
    ...
    if (targetData.userId) targetUserId = targetData.userId;
    else if (targetData.taxistaId) targetUserId = targetData.taxistaId;
    else if (targetData.createdBy) targetUserId = targetData.createdBy;
    ...
    if (!this.canAccessUserData(targetUserId)) return false;
   ...
   */
  getAggregatedDataSummary<T>(data: T[], aggregationField: string = 'amount'): any {
    const currentUser = this.getCurrentUser();
    if (!currentUser || currentUser.rol !== UserRole.PATRON) {
      return null;
    }
    ...
    filteredData.forEach((item: any) => {
      ...
      else if (item.taxistaId) userId = item.taxistaId;
      else if (item.createdBy) userId = item.createdBy;
  ...
  private getStoredAssociations(): Association[] {
    try {
  ...
  private storeAssociations(associations: Association[]): void {
    try {
  ...
  private async notifyAssociationRemoval(association: Association): Promise<void> {
    try {
      const users = this.getStoredUsers();
      const patron = users.find(u => u.id === association.patronId);
      const taxista = users.find(u => u.id === association.taxistaId);
      ...
      const taxistaNotification = {
        ...
        userId: taxista.id,
        type: 'association_removed' as const,
  ...
  async searchAvailableTaxistasAdvanced(options: {
    ...
    try {
      ...
      throw new AuthError(
        AuthErrorCode.NETWORK_ERROR,
        'Error en búsqueda avanzada de taxistas',
        error
      );
    }
  }
  ...
  getAssociationStatistics(): any {
    ...
    try {
      ...
      const recentAssociations = patronAssociations.filter(a => 
        new Date(a.fechaAsociacion) >= thirtyDaysAgo
      );
  ...
  private async notifyTaxistaOfAssociation(taxista: User, patron: User): Promise<void> {
    try {
