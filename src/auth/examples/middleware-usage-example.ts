...
export class SecureDataService {
   ...
   */
  async getUserData(currentUser: User | null, targetUserId: string): Promise<any> {
   ...
   */
  async createAssociation(currentUser: User | null, patronId: string, taxistaId: string): Promise<any> {
    return await this.integration.requirePermission(
      currentUser,
      {
   ...
   */
  async getFilteredData(currentUser: User | null): Promise<any[]> {
    const allData = [
    ...
    return await this.integration.validateAndFilterData(
      currentUser,
      allData,
 ...
 */
export async function demonstrateMiddlewareUsage(): Promise<void> {
  ...
  try {
    ...
    const association = await secureService.createAssociation(
      patronUser,
      patronUser.id,
      taxistaUser.id
    );
    ...
    try {
      await secureService.createAssociation(
        taxistaUser,
        patronUser.id,
        taxistaUser.id
      );
    } catch (error) {
      console.log('✗ Operation denied:', (error as Error).message);
    }
