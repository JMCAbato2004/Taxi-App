...
import { 
  DataSyncBridge,
  createDataSyncBridge,
  SyncAwareServiceData,
  SyncAwareExpenseData,
  SyncDashboard
} from '../integration/data-sync-bridge';
import { 
...
export class DataSyncDemo {
  ...
  constructor(
    ...
    this.syncBridge = createDataSyncBridge(
      secureStorage,
      roleService,
   ...
   */
  private updateNetworkStatus(networkStatus: any, capabilities: any): void {
   ...
   */
  private updateSyncStats(dashboard: SyncDashboard): void {
   ...
   */
  private updateOperationsList(dashboard: SyncDashboard): void {
   ...
   */
  private updateConflictsList(dashboard: SyncDashboard): void {
   ...
   */
  private toggleOfflineMode(): void {
   ...
   * Create demo service
   */
  private async createDemoService(): Promise<void> {
    const serviceData: Partial<SyncAwareServiceData> = {
      id: `service-${Date.now()}`,
    ...
    try {
      ...
      if (result.success) {
        this.showNotification(
          `Service created ${result.willSyncWhenOnline ? '(will sync when online)' : 'and synced'}`
        );
      } else {
   ...
   * Create demo expense
   */
  private async createDemoExpense(): Promise<void> {
    const categories = ['Fuel', 'Maintenance', 'Insurance', 'Parking', 'Tolls'];
    const expenseData: Partial<SyncAwareExpenseData> = {
      id: `expense-${Date.now()}`,
    ...
    try {
      ...
      if (result.success) {
        this.showNotification(
          `Expense created ${result.willSyncWhenOnline ? '(will sync when online)' : 'and synced'}`
        );
      } else {
   ...
   * Update demo profile
   */
  private async updateDemoProfile(): Promise<void> {
    ...
    try {
      ...
      if (result.success) {
        this.showNotification(
          `Profile updated ${result.willSyncWhenOnline ? '(will sync when online)' : 'and synced'}`
        );
      } else {
   ...
   */
  private async createDemoAssociation(): Promise<void> {
    ...
    try {
      ...
      if (result.success) {
        this.showNotification(
          `Association created ${result.willSyncWhenOnline ? '(will sync when online)' : 'and synced'}`
        );
      } else {
   ...
   */
  private async simulateConflict(): Promise<void> {
    ...
    try {
      const result = await this.syncBridge.updateService(
        'conflict-service',
        serviceData,
  ...
  async resolveConflict(conflictId: string, strategy: string): Promise<void> {
    try {
      const resolutionStrategy = strategy as ConflictResolutionStrategy;
...
let dataSyncDemo: DataSyncDemo;

/**
 * Initialize data sync demo
 */
export function initializeDataSyncDemo(
  container: HTMLElement,
  secureStorage: SecureStorageService,
  roleService: RoleService,
  serviceExpenseIntegration: ServiceExpenseIntegrationService,
  ...
  (window as any).dataSyncDemo = dataSyncDemo;
