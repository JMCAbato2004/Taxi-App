...
import { 
  ...
  ConfirmationMethod
} from '../services/sensitive-data-confirmation';
...
export class SensitiveDataConfirmationDemo {
   ...
   */
  async demoPasswordChange(): Promise<void> {
    console.log('\n=== Password Change Demo ===');

    try {
      // Step 1: User initiates password change
      console.log('1. Initiating password change...');
      const confirmationRequest = await this.authService.initiatePasswordChange(
        'CurrentPassword123!',
      ...
      const confirmationResult = await this.sensitiveDataService.processConfirmation(
   ...
   */
  async demoEmailChange(): Promise<void> {
    console.log('\n=== Email Change Demo ===');

    try {
      // Step 1: User initiates email change
      console.log('1. Initiating email change...');
      const confirmationRequest = await this.authService.initiateEmailChange(
      ...
      const passwordResult = await this.sensitiveDataService.processConfirmation(
        confirmationRequest.id,
        {
          method: ConfirmationMethod.PASSWORD_VERIFICATION,
          data: { password: 'CurrentPassword123!' }
      ...
      if (passwordResult.success) {
        ...
        const emailResult = await this.sensitiveDataService.processConfirmation(
          confirmationRequest.id,
          {
   ...
   */
  async demoAssociationManagement(): Promise<void> {
    ...
    try {
      ...
      const createRequest = await this.roleService.initiateAssociationCreation(
        patronId,
        taxistaId
      );
      ...
      const createResult = await this.sensitiveDataService.processConfirmation(
        createRequest.id,
        {
          method: ConfirmationMethod.PASSWORD_VERIFICATION,
      ...
      if (createResult.success && createResult.canProceed) {
        ...
        const association = await this.roleService.executeConfirmedAssociationCreation(
          createRequest.id
        );
        ...
        const removeRequest = await this.roleService.initiateAssociationRemoval(
          association.id
        );
        ...
        const removeResult = await this.sensitiveDataService.processConfirmation(
          removeRequest.id,
          {
            method: ConfirmationMethod.PASSWORD_VERIFICATION,
            data: { password: 'PatronPassword123!' }
   ...
   */
  async demoConfirmationStatus(): Promise<void> {
    console.log('\n=== Confirmation Status Demo ===');

    try {
      // Create a confirmation request
      const request = await this.authService.initiatePasswordChange(
        'CurrentPassword123!',
        'NewPassword456!'
      );
      ...
      let status = await this.sensitiveDataService.getConfirmationStatus(
        request.id,
        this.authService.getCurrentUser()!
      );
      ...
      await this.sensitiveDataService.processConfirmation(
        request.id,
        {
          method: ConfirmationMethod.PASSWORD_VERIFICATION,
          data: { password: 'CurrentPassword123!' }
      ...
      status = await this.sensitiveDataService.getConfirmationStatus(
        request.id,
        this.authService.getCurrentUser()!
      );
      ...
      await this.sensitiveDataService.cancelConfirmation(
        request.id,
        this.authService.getCurrentUser()!
      );
 ...
 * Usage example
 */
export async function runSensitiveDataDemo(): Promise<void> {
