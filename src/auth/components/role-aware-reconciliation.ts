...
export class RoleAwareReconciliationModule {
  private reconciliationIntegration: ReconciliationIntegrationService;
  private originalStorageManager: any;
  private roleAwareStorageManager: any;

  constructor(
    private authService: AuthService,
    private roleService: RoleService,
    originalStorageManager: any
  ) {
    this.reconciliationIntegration = new ReconciliationIntegrationService(
      this.roleService,
   ...
   */
  getAggregatedSummary(dataType: 'services' | 'expenses' | 'reconciliations' = 'services') {
   ...
   */
  generateRoleAwareReconciliation(
  ...
  getRoleSpecificStatistics() {
    ...
    const baseStats = {
      ...
      totalServiceAmount: services.reduce((sum: number, s: any) => sum + (s.totalAmount || 0), 0),
      totalExpenseAmount: expenses.reduce((sum: number, e: any) => sum + (e.amount || 0), 0)
    };
  ...
  validateReconciliationOperation(operation: 'create' | 'read' | 'update' | 'delete', targetData?: any): boolean {
    ...
    switch (operation) {
      ...
      case 'read':
        if (!targetData) return true; // Can read own filtered data
   ...
   */
  exportRoleFilteredData(format: 'json' | 'csv' = 'json') {
    const currentUser = this.authService.getCurrentUser();
    if (!currentUser) {
      throw new AuthError(
        AuthErrorCode.SESSION_EXPIRED,
 ...
 */
export function createRoleAwareReconciliationModule(
  authService: AuthService,
  roleService: RoleService,
  originalStorageManager: any
): RoleAwareReconciliationModule {
 ...
 */
export function enhanceReconciliationWithRoles(
  originalReconciliationModule: any,
  authService: AuthService,
  roleService: RoleService
) {
  const roleAwareModule = createRoleAwareReconciliationModule(
    authService,
    roleService,
    originalReconciliationModule.storageManager || window.ReconciliationStorageManager
  );
  ...
  return {
    ...
    getAggregatedSummary: (dataType?: 'services' | 'expenses' | 'reconciliations') => 
      roleAwareModule.getAggregatedSummary(dataType),
    ...
    validateOperation: (operation: 'create' | 'read' | 'update' | 'delete', targetData?: any) =>
      roleAwareModule.validateReconciliationOperation(operation, targetData),
    getNotifications: () => roleAwareModule.getRoleSpecificNotifications(),
    exportData: (format?: 'json' | 'csv') => roleAwareModule.exportRoleFilteredData(format)
  };
}