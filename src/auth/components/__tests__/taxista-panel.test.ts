...
import { TaxistaPanel, TaxistaPanelConfig } from '../taxista-panel';
...
import { UserRole, AuthError, AuthErrorCode, TaxistaUser } from '../../types';

describe('TaxistaPanel', () => {
  let authService: AuthService;
  let roleService: RoleService;
  let serviceExpenseService: ServiceExpenseIntegrationService;
  let reconciliationService: ReconciliationIntegrationService;
  let taxistaPanel: TaxistaPanel;
  let config: TaxistaPanelConfig;

  // Mock taxista user
  const mockTaxistaUser: TaxistaUser = {
    id: 'taxista_123',
  ...
  beforeEach(() => {
    ...
    serviceExpenseService = new ServiceExpenseIntegrationService(
      roleService,
  ...
  describe('Initialization', () => {
    ...
    it('should throw error when initialized with non-taxista user', () => {
      ...
      jest.spyOn(authService, 'getCurrentUser').mockReturnValue(patronUser as any);
  ...
  describe('Error Handling', () => {
    ...
    it('should handle service creation errors gracefully', async () => {
      ...
      jest.spyOn(serviceExpenseService, 'createService').mockRejectedValue(
        new AuthError(AuthErrorCode.VALIDATION_ERROR, 'Invalid service data')
      );
    ...
    it('should handle expense creation errors gracefully', async () => {
      ...
      jest.spyOn(serviceExpenseService, 'createExpense').mockRejectedValue(
        new AuthError(AuthErrorCode.VALIDATION_ERROR, 'Invalid expense data')
      );
