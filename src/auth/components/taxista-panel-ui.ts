...
import { 
  TaxistaPanel, 
  TaxistaPanelConfig, 
  TaxistaPersonalStats,
  TaxistaPersonalProfile,
  HistoryFilters
} from './taxista-panel';
import { 
  TaxistaUser,
...
export class TaxistaPanelUI {
  private panel: TaxistaPanel;
  private container: HTMLElement | null = null;
  private theme: any;

  constructor(private config: TaxistaPanelUIConfig) {
    this.theme = {
      primaryColor: '#059669',
   ...
   */
  private renderPersonalSummary(): string {
   ...
   */
  private renderServiceCard(service: AuthenticatedServiceData): string {
    return `
   ...
   */
  private renderExpenseCard(expense: AuthenticatedExpenseData): string {
    return `
   ...
   */
  async applyHistoryFilters(): Promise<void> {
    const dateFrom = (document.getElementById('dateFrom') as HTMLInputElement)?.value;
    const dateTo = (document.getElementById('dateTo') as HTMLInputElement)?.value;
    const type = (document.getElementById('historyType') as HTMLSelectElement)?.value as any;
  ...
  clearHistoryFilters(): void {
    ...
    (document.getElementById('historyType') as HTMLSelectElement).value = 'all';
  ...
  async createService(): Promise<void> {
    const serviceType = (document.getElementById('serviceType') as HTMLSelectElement)?.value;
    const amount = parseFloat((document.getElementById('serviceAmount') as HTMLInputElement)?.value || '0');
    const origin = (document.getElementById('serviceOrigin') as HTMLInputElement)?.value;
    const destination = (document.getElementById('serviceDestination') as HTMLInputElement)?.value;
    const distance = parseFloat((document.getElementById('serviceDistance') as HTMLInputElement)?.value || '0');
    const duration = parseInt((document.getElementById('serviceDuration') as HTMLInputElement)?.value || '0');
  ...
  async createExpense(): Promise<void> {
    ...
    const amount = parseFloat((document.getElementById('expenseAmount') as HTMLInputElement)?.value || '0');
    const vendor = (document.getElementById('expenseVendor') as HTMLInputElement)?.value;
    const description = (document.getElementById('expenseDescription') as HTMLTextAreaElement)?.value;
  ...
  async updateSettings(): Promise<void> {
    ...
    const dataSharing = (document.getElementById('dataSharing') as HTMLInputElement)?.checked || false;
    const autoSync = (document.getElementById('autoSync') as HTMLInputElement)?.checked || false;
   ...
   */
  private updateHistoryResults(): void {
  ...
  private attachEventListeners(): void {
    ...
    (window as any).taxistaPanelUI = this;
 ...
 */
export function createTaxistaPanelUI(config: TaxistaPanelUIConfig): TaxistaPanelUI {
