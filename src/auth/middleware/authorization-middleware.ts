...
export class AuthorizationMiddleware {
  private readonly ACCESS_LOG_KEY = 'taxi_access_log';
    ...
    try {
      ...
      if (!hasPermission) {
        const error = new AuthError(
          ...
          `Sin permisos para ${context.action} en ${context.resource}`
        );
   ...
   */
  async encryptSensitiveData<T extends Record<string, any>>(
    data: T,
    ...
    try {
      ...
      for (const field of config.encryptFields) {
        if (encryptedData[field] && typeof encryptedData[field] === 'string') {
          encryptedData[field] = await this.cryptoUtils.encryptSensitiveData(
            encryptedData[field],
   ...
   */
  async decryptSensitiveData<T extends Record<string, any>>(
    data: T,
    config: EncryptionConfig
  ): Promise<T> {
    if (config.skipEncryption || config.encryptFields.length === 0) {
    ...
    try {
      ...
      for (const field of config.encryptFields) {
        if (decryptedData[field] && typeof decryptedData[field] === 'string') {
          decryptedData[field] = await this.cryptoUtils.decryptSensitiveData(
            decryptedData[field],
            config.encryptionKey
          );
        }
   ...
   */
  getSecurityStatistics(timeframe: 'day' | 'week' | 'month' = 'day'): {
    ...
    try {
      ...
      const failedResourceCounts: Record<string, number> = {};
      recentLogs
      ...
      const suspiciousActivity = recentLogs.filter(log => 
        log.userId && userFailureCounts[log.userId] >= 5
      );
   ...
   */
  cleanupAccessLog(maxAge: number = 30): number {
    try {
      const logs = this.getStoredAccessLog();
  ...
  private createLogEntry(context: AuthorizationContext): AccessAttempt {
    return {
  ...
  private async logAccessAttempt(logEntry: AccessAttempt): Promise<void> {
    try {
      const logs = this.getStoredAccessLog();
      logs.push(logEntry);
  ...
  private isOwnData(user: User, targetData: any): boolean {
    if (!targetData) return false;

    // Check common patterns for user ownership
    return (
      targetData.userId === user.id ||
  ...
  private async canPatronAccessData(patron: User, targetData: any): Promise<boolean> {
    ...
    try {
      ...
      const patronAssociations = associations.filter(a => 
        a.patronId === patron.id && a.activa
      );
      ...
      return (
        (targetData.userId && associatedTaxistaIds.includes(targetData.userId)) ||
  ...
  private isAssociatedTaxistaNumber(patronId: string, numeroTaxista: string): boolean {
    try {
      const users = this.getStoredUsers();
      const taxista = users.find(u => u.numeroTaxista === numeroTaxista);
      
      if (!taxista) return false;
  ...
  private getStoredAccessLog(): AccessAttempt[] {
    try {
  ...
  private storeAccessLog(logs: AccessAttempt[]): void {
    try {
  ...
  private getStoredAssociations(): any[] {
    try {
